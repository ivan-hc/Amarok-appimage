...
DEPENDENCES="alsa-lib dbus ibus libibus libpulse nss udisks2 pipewire phonon-qt6 phonon-qt6-vlc libvlc vlc" #SYNTAX: "APP1 APP2 APP3 APP4...", LEAVE BLANK IF NO OTHER DEPENDENCIES ARE NEEDED
...
LIBSAVED="libsodium.so libsoxr.so libproxy libduktape.so libFLAC.so libogg libmp3lame libvorbis libnghttp libssh libpsl.so libxcb qt libuuid \
libGLX libglapi.so libXxf libSPIRV-Tools.so libsensors.so libxshmfence.so libdrm libpciaccess.so \
svg mysql libnss_mdns gdk-pixbuf json $lib_audio_keywords $lib_browser_launcher"
...
_extract_package() {
	...
	if test -f "$pkg_full_path"; then
		...
		[ -n "$lib_browser_launcher" ] && [[ "$arg" =~ (hicolor-icon-theme|xapp|kio|udisks2|qt6-svg|phonon-qt6|phonon-qt6-vlc|libvlc|vlc) ]] && tar fx "$pkg_full_path" -C ./base/ --warning=no-unknown-keyword --exclude='.PKGINFO'
	fi
}
...
_remove_more_bloatwares() {
	...
	rm -Rf ./"$APP".AppDir/.junest/usr/lib/lib*WebEngineCore*
