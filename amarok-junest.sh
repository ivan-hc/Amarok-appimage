...
DEPENDENCES="ca-certificates freetype2 harfbuzz ifuse kcmutils5 kded-git kdnssd5 kdoctools5 kio-git kio-extras kirigami2 knewstuff5 ktexteditor5 libglvnd liblastfm-qt5 libofa loudmouth mariadb mariadb-clients mariadb-libs mesa phonon-qt5 phonon-qt5-vlc pulseaudio qt5-webengine threadweaver5"
...
SHARESAVED="certificates qt db kf" # Enter here keywords or file/folder names to save in both /usr/share and /usr/lib
LIBSAVED="pk p11 alsa jack pipewire pulse qt db libGLX libglapi libxcb libXx libxshmfence vlc libXcursor libqnmbearer mysql Qt libnss p11 kf" # Enter here keywords or file/folder names to save in /usr/lib
